document.addEventListener('DOMContentLoaded', () => {
    ...
    function randomApple() {
        ...
        } while(squares[appleIndex].classList.contains('snake')) //no apples on snake
        squares[appleIndex].classList.add('apple')
    }

    //assign movement to keyboard keys
    function control(e) {
        squares[currentIndex].classList.remove('snake')

        if(e.keyCode === 39) {
            direction = 1 //right arrow moves right
        } else if (e.keyCode === 38) {
            direction = -width //up arrow moves up
        } else if (e.keyCode === 37) {
            direction = -1 //left arrow moves left
        } else if (e.keyCode === 40) {
            direction = +width //down arrow moves down
        }
    }

    document.addEventListener('keyi')
})